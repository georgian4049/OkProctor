...
$course_name_fetch = mysqli_query($conn,"SELECT `sub_name` FROM `course_name` where `sub_code`='$courses_array[6]' ");
                      $course_name_fetched=mysqli_fetch_assoc($course_name_fetch);
                      $course_name_val=$course_name_fetched['sub_name'];

$teachers_fetch=mysqli_query($conn,"SELECT * from `teachers` where `co1`='$courses_array[6]' or `co2`='$courses_array[6]' or `lab`='$courses_array[6]' and `branch`='$branch' ");
